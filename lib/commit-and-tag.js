...
nodegit.Repository.open(path.resolve(process.cwd(), ".git")).then(repo => repo.refreshIndex().then(index => index.addAll().then(() => index.write()).then(() => index.writeTree()).then(oid => nodegit.Reference.nameToId(repo, "HEAD").then(head => repo.getCommit(head)).then(parent => {
